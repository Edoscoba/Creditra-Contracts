...
impl Credit {
    ...
    /// Transfers the protocol token from borrower to the contract via allowance + transfer_from.
...
mod test {
    ...
    fn setup_contract_with_credit_line<'a>(
        ...
        (client, token_address, admin, contract_id)
    }
    ...
    fn test_draw_transfers_correct_amount_to_borrower() {
        ...
        let (client, token_address, _admin, _contract_id) =
    ...
    fn test_draw_updates_utilized_amount() {
        ...
        env.mock_all_auths();
        let borrower = Address::generate(&env);
        let (client, _token, _admin, _contract_id) =
            setup_contract_with_credit_line(&env, &borrower, 1_000, 1_000);
    ...
    fn test_draw_on_closed_line_fails() {
        ...
        env.mock_all_auths();
        let borrower = Address::generate(&env);
        let (client, _token, admin, _contract_id) =
            setup_contract_with_credit_line(&env, &borrower, 1_000, 1_000);
    ...
    fn test_repay_credit_reduces_utilized_amount() {
        ...
        env.mock_all_auths();
        let borrower = Address::generate(&env);
        let (client, token_address, _admin, contract_id) =
            setup_contract_with_credit_line(&env, &borrower, 1_000, 1_000);
        client.draw_credit(&borrower, &500);
        approve_token_spend(&env, &token_address, &borrower, &contract_id, 200);
    ...
    fn test_repay_credit_transfers_token_and_consumes_allowance() {
        ...
        let token_client = token::Client::new(&env, &token_address);
        approve_token_spend(&env, &token_address, &borrower, &contract_id, 600);
        ...
        assert_eq!(
            ...
            borrower_balance_before - 200
        );
        assert_eq!(
            token_client.balance(&contract_id),
            contract_balance_before + 200
        );
        assert_eq!(
            token_client.allowance(&borrower, &contract_id),
            allowance_before - 200
        );
        assert_eq!(
            client.get_credit_line(&borrower).unwrap().utilized_amount,
            100
    ...
    fn test_repay_credit_reverts_on_insufficient_balance() {
        ...
        // Spend tokens so borrower's balance drops below the repayment amount.
        let token_client = token::Client::new(&env, &token_address);
        token_client.transfer(&borrower, &recipient, &250);
    ...
    fn test_repay_credit_saturates_at_zero() {
        ...
        let (client, token_address, _admin, contract_id) =
            setup_contract_with_credit_line(&env, &borrower, 1_000, 1_000);
        client.draw_credit(&borrower, &100);
        // Only 100 is utilized, so only 100 is transferred even if amount is higher.
        approve_token_spend(&env, &token_address, &borrower, &contract_id, 100);
...
mod test_close_utilized {
    ...
    fn test_close_utilized_borrower_rejected_at_minimum_utilization() {
        ...
        env.mock_all_auths();
        let borrower = Address::generate(&env);
        let (client, _admin, _token, _contract_id) = setup(&env, &borrower, 1_000, 1_000);
    ...
    fn test_close_utilized_borrower_rejected_at_full_utilization() {
        ...
        env.mock_all_auths();
        let borrower = Address::generate(&env);
        let (client, _admin, _token, _contract_id) = setup(&env, &borrower, 500, 500);
        client.draw_credit(&borrower, &500);
        client.close_credit_line(&borrower, &borrower);
    }
    ...
    fn test_close_utilized_admin_force_close_preserves_utilized_amount() {
        ...
        env.mock_all_auths();
        let borrower = Address::generate(&env);
        let (client, admin, _token, _contract_id) = setup(&env, &borrower, 1_000, 1_000);
    ...
    fn test_close_utilized_borrower_succeeds_after_full_repayment() {
        ...
        env.mock_all_auths();
        let borrower = Address::generate(&env);
        let (client, _admin, token_address, contract_id) = setup(&env, &borrower, 1_000, 1_000);
        client.draw_credit(&borrower, &350);
        approve_token_spend(&env, &token_address, &borrower, &contract_id, 350);
    ...
    fn test_close_utilized_third_party_rejected_with_zero_utilization() {
        ...
        let (client, _admin, _token, _contract_id) = setup(&env, &borrower, 1_000, 0);
